...
const filterEuro = document.querySelector('#eurobeat_check');				//tuve que hacer 2 listeners, 1 para cada filtro. no sé si hay una mejor manera.
const filterTiktok = document.querySelector('#tiktok_check');

filterEuro.addEventListener("change", () => {
	var estado_check = document.getElementById("eurobeat_check").checked;
...
filterTiktok.addEventListener("change", () => {
	var estado_check = document.getElementById("tiktok_check").checked;			//basicamente, cuando cambia la caja hace un checkeo de si esta marcada o no
																				//luego ejecuta la funcion que esta definida más abajo. hideDiv y unHideDiv
	if(!estado_check){
		hideDiv("tiktok");
		}else{
		unHideDiv("tiktok");
...
function hideDiv(div_class){
	var x = document.getElementsByClassName(div_class);
	for (var i = 0; i < x.length; i ++) {
    x[i].style.display = 'none';
	}
}

function unHideDiv(div_class){
	var x = document.getElementsByClassName(div_class);
	for (var i = 0; i < x.length; i ++) {
